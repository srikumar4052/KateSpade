...
import com.ch.ksd.bean.Databean;
import com.ch.ksd.bean.ForgotPwdDatabean;
import com.ch.ksd.bean.ImageTextBean;
import com.ch.ksd.bean.InvalidAboutYou;
import com.ch.ksd.bean.InvalidCreateAccount;
import com.ch.ksd.bean.LoadBean;
import com.ch.ksd.bean.SignUpDataBean;
import com.ch.ksd.bean.User;
import com.ch.ksd.bean.VehicleSelectorBean;
...
public class FileUtility implements FileConstants {
	...
	public static ArrayList<com.ch.ksd.bean.InvalidSignInbean> readAllInvalidSignInData() throws Exception {
		BufferedReader reader = new BufferedReader(new FileReader(INVALID_SIGNIN_USER_FILE));
		ArrayList<com.ch.ksd.bean.InvalidSignInbean> data = new ArrayList<com.ch.ksd.bean.InvalidSignInbean>();
		String line = reader.readLine();
		line = reader.readLine();
		while (null != line) {
			String[] records = line.split(";");
			if (records.length >= 3) {
				data.add(new com.ch.ksd.bean.InvalidSignInbean(records));
			}
			line = reader.readLine();
		}
		reader.close();
